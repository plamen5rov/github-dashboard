...
- **Fix:** Export `OSI_LICENSE_IDS` Set from `licenseLegend.ts`; import in `utils.ts` instead of inline Set
...
- **Fix:** Use `useRef` for the timer and onChange callback; remove `useCallback`
